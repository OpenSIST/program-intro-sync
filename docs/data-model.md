...
## Deprecated File-State Types

These shapes are useful for tests and local fixtures, but production monitor state should live in D1, not JSON files.

### SourceState

Committed state that tracks source hashes and prior match decisions.
...
type SourceStateEntry = {
  ...
  matchedProgramId: string | null;
  matchConfidence: number;
  descriptionHashAtDraft?: string;
  lastDraftHash?: string;
  status: "new" | "changed" | "merged" | "ignored" | "needs_review";
  lastSeenAt: string;
};
```

The state should not store full markdown source content.

### ChangeSet

Detected changes for a run.
...
type ChangeSet = {
  ...
  summary: {
    ...
    matchChanged: number;
    openSistDescriptionChanged: number;
...
type ChangeEntry = {
  ...
  sourceKey: string;
  matchedProgramId: string | null;
  previous?: SourceStateEntry;
  current?: MatchedSourceDoc;
};
```

## Downstream LLM Types

These are intentionally outside the monitor core.

### MergeInput

Input to the LLM provider.

```ts
type MergeInput = {
  program: OpenSistProgram;
  currentDescriptionMarkdown: string;
  changedSources: MatchedSourceDoc[];
  relatedSources: MatchedSourceDoc[];
  policy: {
...
Structured LLM output after validation.
...
type MergeResult = {
  ...
  conflicts: Array<{
    ...
    existing: string;
    incoming: string;
    recommendation: "needs_review" | "skip" | "manual_merge";
  }>;
...
Draft Markdown can include frontmatter:
